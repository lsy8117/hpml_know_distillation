...
class ProfiledGRPOTrainer(GRPOTrainer):
    ...
    # pass precise_timing down to the profiler when creating it
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        self._profiler         = GRPOStepProfiler(device, precise_timing=self.precise_timing)  # ← pass through
    ...
    def _should_profile(self) -> bool:
        step = self.state.global_step
        return self.profile_start_step <= step < self.profile_start_step + self.profile_steps
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        step = self.state.global_step
        device = next(model.parameters()).device
        self._profiler = GRPOStepProfiler(device)
        self._profiling_active = True
